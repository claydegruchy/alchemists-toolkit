...
var pm = new PotionMaster.PotionMaster(1);
...
<select class="multiple ingredients-selector" multiple>
...
var potionTemplate = `
produced potion:
<div class="potion" id="{{potion._name}}">
    <h3>{{potion._name}}</h3>
	<div class="image-container">
	</div>
    <h3>Active effects</h3>
    {{#each activeEffects}}
	    <li>{{this.name}} - {{this.strength}}</li>
    {{else}}
	    Potion has no effects
    {{/each}}
    <h3>Passive effects</h3>
    {{#each passiveEffects}}
	    <li>{{this.name}}</li>
    {{else}}
	    Potion has no passive effects
    {{/each}}
    <h3>Potion properties</h3>
    {{#each potion._properties}}
	    <li>{{this}}</li>
    {{else}}
...
`;
var ingredientSelector = Handlebars.compile(ingredientSelectorTemplate);
...
function updateTemplate() {
    document.getElementById(
        "selectionContainer"
    ).innerHTML = ingredientSelector({
        components: pm.components(),
        productionEffects: pm.productionEffects()
    });
...
document.getElementById("start").onclick = e => {
    ...
    document.getElementById("output").innerHTML = potionTemplate({
        potion: potion,
        activeEffects: potion.effectsAdvanced().filter(e => e.active),
        passiveEffects: potion.effectsAdvanced().filter(e => !e.active)
    });
    console.log(potion._color);
    var potionBottle = SVG(
    ...
    // .fill({ color: potion.colour.hex(), opacity: potion.colour.alpha() });
